...
struct StreamerDiagnostics {double time{},dt{},emax{},ne_max{},np_max{},nn_max{},total_electrons{},total_charge{},conservation_residual{},applied_voltage{},phi_hv_residual{},phi_ground_residual{},gap{},tip_radius{},sigma_max{},head_position{},head_velocity{},absorbed_electron_hv{},absorbed_electron_ground{};bool bridge_flag{};int poisson_iterations{},sp3_ksp_iterations{},sp3_boundary_iterations{};std::string controller,geometry_id;};
...
class StreamerSolver {public: StreamerSolver(const AxisymmetricGrid&,StreamerConfig);void initialize_gaussian(double n0,double sigma,double z0);void initialize_gaussian_at_tip_offset(double n0,double sigma,double z_offset_from_tip);void initialize_gaussians(const std::vector<GaussianSeed>& seeds);TimeStepLimits timestep_limits()const;ReactionTimestepDiagnostic reaction_timestep_diagnostic()const;double numerical_density_tolerance()const;bool step(double dt,StreamerDiagnostics&);void save_checkpoint(const std::filesystem::path&)const;void load_checkpoint(const std::filesystem::path&);const StreamerState&state()const{return state_;}StreamerState&state(){return state_;}private:const AxisymmetricGrid&g_;StreamerConfig c_;StreamerState state_;Sp3WarmStart sp3_warm_;double last_head_position_{std::numeric_limits<double>::quiet_NaN()},last_head_time_{std::numeric_limits<double>::quiet_NaN()};bool electrode_mode()const{return c_.electrode_geometry&&c_.voltage_waveform;}ElectrodeCellType cell_type(int i,int j)const;bool is_gas(int i,int j)const{return cell_type(i,j)==ElectrodeCellType::Gas;}double stage_c_activity_tolerance()const;void enforce_plasma_mask();void enforce_stage_c_activity_floor();void fields();void fill_electrode_diagnostics(StreamerDiagnostics&)const;double head_position()const;bool bridge_flag()const;};
}
