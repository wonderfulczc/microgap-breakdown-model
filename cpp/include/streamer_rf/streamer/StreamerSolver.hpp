...
struct StreamerDiagnostics {double time{},dt{},emax{},ne_max{},np_max{},nn_max{},total_electrons{},total_charge{},conservation_residual{},applied_voltage{},phi_hv_residual{},phi_ground_residual{},gap{},tip_radius{};int poisson_iterations{},sp3_ksp_iterations{},sp3_boundary_iterations{};std::string controller,geometry_id;};
struct GaussianSeed {double n0{},sigma{},z0{};};
class StreamerSolver {public: StreamerSolver(const AxisymmetricGrid&,StreamerConfig);void initialize_gaussian(double n0,double sigma,double z0);void initialize_gaussian_at_tip_offset(double n0,double sigma,double z_offset_from_tip);void initialize_gaussians(const std::vector<GaussianSeed>& seeds);TimeStepLimits timestep_limits()const;bool step(double dt,StreamerDiagnostics&);void save_checkpoint(const std::filesystem::path&)const;void load_checkpoint(const std::filesystem::path&);const StreamerState&state()const{return state_;}StreamerState&state(){return state_;}private:const AxisymmetricGrid&g_;StreamerConfig c_;StreamerState state_;Sp3WarmStart sp3_warm_;void fields();void fill_electrode_diagnostics(StreamerDiagnostics&)const;};
}
